...
useEffect(()=>{  
    figureitemsToShow()
    if(typeIsChosen){
    setUpperTitle('Чуть конкретнее, пожалуйста')
    }
    else{
...
useEffect(()=>{  
    figureCustom()
    if(itemIsChosen){
        setUpperTitle('Подстройте блюдо под себя!')
        }
...
<Title small>Количество</Title>
        ...
        yellowShadow>
            ...
            <Button accent onClick={reset}>Да, верните меня назад</Button>
           <Button href="#4" onClick={reset}>Нет, давайте двигаться дальше</Button>
        </Container>
    </Container>
</Section>
